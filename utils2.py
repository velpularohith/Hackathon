...
def store_feedback(query):
    ...
    insert_query = f"INSERT INTO PHOENIX_DB.PHOENIX_SC.FEEDBACK(FEEDBACK) VALUES ('{query}')"
...
def store_ticket(task_title,task_description,task_ac,task_assignee,task_status,task_priority,task_comments):
    ...
    insert_query = """
        ...
        (TITLE, DESCRIPTION, ACCEPTANCE_CRITERIA,STATE,PRIORITY,COMMENTS)
        VALUES (%s, %s, %s, %s, %s, %s)
    """
    cursor.execute(insert_query, (task_title, task_description, task_ac,task_status,task_priority,task_comments))

    cursor.close()
    
####
def store_code(task_title,task_description,task_ac,task_assignee,task_status,task_priority,task_code):
    cursor = connection.cursor()
    # insert_query = f"INSERT INTO PHOENIX_DB.PHOENIX_SC.JIRA_TICKETS(TITLE,DESCRIPTION,ACCEPTANCE_CRITERIA) VALUES ('{task_title}','{task_description}','{task_ac}');"
    # cursor.execute(insert_query)

    insert_query = """
        INSERT INTO PHOENIX_DB.PHOENIX_SC.TICKET_INFO
        (TITLE, DESCRIPTION, ACCEPTANCE_CRITERIA,STATE,PRIORITY,CODE)
        VALUES (%s, %s, %s, %s, %s, %s)
    """
    cursor.execute(insert_query, (task_title, task_description, task_ac,task_status,task_priority,task_code))
